...
def create_stdp_synapse_model():
    """
    Create STDP synapse model with both potentiation and depression.
    
    STDP Rule:
    - Pre before Post (Δt > 0): Potentiation (LTP)
    - Post before Pre (Δt < 0): Depression (LTD)
    
    Δw = A+ * exp(-Δt/τ+) for Δt > 0 (potentiation)
    Δw = -A- * exp(Δt/τ-) for Δt < 0 (depression)
    """
    
    # STDP Parameters (based on Bi & Poo, 1998)
    stdp_params = {
        'tau_pre': 20*ms,      # Time constant for pre-synaptic trace
        'tau_post': 20*ms,     # Time constant for post-synaptic trace
        'A_plus': 0.01,        # Potentiation amplitude
        'A_minus': 0.012,      # Depression amplitude (slightly larger for stability)
        'w_max': 1.0,          # Maximum synaptic weight
        'w_min': 0.0,          # Minimum synaptic weight
    }
    
    # Neuron model equations (LIF)
    neuron_eqs = '''
    dv/dt = (v_rest - v + I_ext) / tau_m : volt
    I_ext : volt
    '''
    
    # Synapse model with STDP traces
    synapse_eqs = '''
    w : 1                           # Synaptic weight
    dapre/dt = -apre / tau_pre : 1  # Pre-synaptic trace
    dapost/dt = -apost / tau_post : 1  # Post-synaptic trace
    '''
    
    # On pre-synaptic spike: increase pre-trace, apply depression
    on_pre = '''
    v_post += w * 10*mV
    apre += A_plus
    ...
    # On post-synaptic spike: increase post-trace, apply potentiation
    on_post = '''
    apost += A_minus
...
#%% Part 2: Simulate STDP Window (-100ms to +100ms)
print("\n" + "=" * 60)
...
def simulate_stdp_window(timing_intervals, n_pairs=50, initial_weight=0.5):
    """
    Simulate STDP for various pre-post timing intervals.
    
    Parameters:
    -----------
    timing_intervals : array
        Time differences (t_post - t_pre) in ms
    n_pairs : int
        Number of spike pairs per timing interval
    initial_weight : float
        Starting synaptic weight
        
    Returns:
    --------
    weight_changes : array
        Change in synaptic weight for each timing interval
    """
    ...
    for delta_t in timing_intervals:
        # Start fresh for each timing interval
        start_scope()
        ...
        # Create spike times for pre and post neurons
        # Use 50ms interval between pairs to avoid interference
        pair_interval = 200  # ms between pairs
...
timing_intervals = np.linspace(-100, 100, 41)  # 5ms resolution

print("Simulating STDP window with 50 spike pairs per timing interval...")
print(f"Timing range: {timing_intervals[0]} ms to {timing_intervals[-1]} ms")

weight_changes = simulate_stdp_window(timing_intervals, n_pairs=50)
...
# Fit positive (potentiation) side
...
plt.tight_layout()
plt.savefig('stdp_learning_window.png', dpi=150, bbox_inches='tight')
plt.show()
...
def simulate_correlated_network():
    """
    Simulate a network of 10 neurons receiving correlated inputs.
    Demonstrate that STDP strengthens connections between co-active neurons.
    """
    start_scope()
    ...
    # Create input neurons with correlated activity
    # Group 1 (inputs 0-49): correlated with neurons 0-4
    # Group 2 (inputs 50-99): correlated with neurons 5-9
    
    input_rates = 20 * Hz  # Base firing rate
    ...
    group_id : integer  # 0 for first group, 1 for second group
    '''
    
    neurons = NeuronGroup(N, neuron_eqs, threshold='v > v_thresh',
                         reset='v = v_reset', method='euler')
    neurons.v = v_rest
    neurons.group_id = [0]*5 + [1]*5  # First 5 in group 0, last 5 in group 1
    
    # Create synapses from input to neurons with STDP
    synapse_eqs = '''
    w : 1
    ...
    # Inputs 0-49 connect strongly to neurons 0-4
    # Inputs 50-99 connect strongly to neurons 5-9
    synapses.connect()  # All-to-all initially
    
    # Initialize weights based on correlation structure
    initial_weights = np.zeros((N_input, N))
    for i in range(N_input):
        for j in range(N):
            if (i < 50 and j < 5) or (i >= 50 and j >= 5):
                initial_weights[i, j] = 0.3  # Correlated pairs start moderate
            else:
                initial_weights[i, j] = 0.3  # Same initial weight for fair comparison
    
    synapses.w = initial_weights.flatten()
    
    # Also create lateral synapses between neurons with STDP
    lateral_eqs = '''
    w : 1
    dapre/dt = -apre / tau_pre : 1 (event-driven)
    dapost/dt = -apost / tau_post : 1 (event-driven)
    ...
    lateral_synapses.connect(condition='i != j')  # No self-connections
    lateral_synapses.w = 0.2  # Start with uniform weights
...
# Plot 2: Final lateral weight matrix
ax2 = axes[0, 1]
...
# Plot 3: Weight evolution for within-group vs between-group
ax3 = axes[1, 0]

# Calculate average within-group and between-group weights over time
...
plt.tight_layout()
plt.savefig('stdp_network_results.png', dpi=150, bbox_inches='tight')
plt.show()
...
correlations = calculate_spike_correlations(spike_mon, N)
...
# Plot correlation vs final weight
ax2 = axes[1]
corr_values = []
weight_values = []
...
ax2.plot(x_line, p(x_line), 'r--', linewidth=2, label=f'Linear fit')
ax2.legend()

plt.tight_layout()
plt.savefig('stdp_correlation_analysis.png', dpi=150, bbox_inches='tight')
plt.show()
